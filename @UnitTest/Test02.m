...
% UNIT TEST 2: Plastimatch rigid registration accuracy
%
% DESCRIPTION: This unit test runs a series of random registration
% adjustments against RigidRegister using the Plastimatch algorithm. Using
% a randomly generated registration vector, each reference image is
...
testCase.StoreResults('summary', ...
    sprintf('Plastimatch rigid registration RMS error (n = %i)', ...
...
for i = 1:size(testCase.inputData, 1)
    ...
    for j = 1:n
        ...
        r = RigidRegister(image, mod, 'method', 'PLASTIMATCH');
        
        % Store errors
        if ~isempty(errors)
            errors = vertcat(errors, reshape((r + v)', 3, 2)); %#ok<AGROW>
        else
            errors = reshape((r + v)', 3, 2);
        end
    end
end
